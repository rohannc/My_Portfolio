...
// Handle scroll behavior: hide during scroll, show when stopped, clear highlights
const handleScroll = () => {
...
const handleKeydown = (event) => {
    if (event.ctrlKey && event.key === 'k') {
        ...
        const input = document.getElementById(
            isDesktop ? 'search-navbar-desktop' : 'search-navbar-mobile'
        );
        if (input) {
            input.focus();
        ...
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
            ...
            <div class="flex items-center md:order-2">
                <div class="relative hidden md:block">
                    ...
                    <input type="text" id="search-navbar-desktop" v-model="searchQuery" @focus="isSearchFocused = true"
                        @blur="isSearchFocused = false" class="block w-full p-2 ps-10 pr-12 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50
                        focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400
                        dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Type to Search..." />
                    <!-- Right-Aligned Shortcut Indicator -->
                    <div class="absolute inset-y-0 end-0 flex items-center pe-3 pointer-events-none">
                        <img src="https://img.icons8.com/?size=100&id=zP1mYl1gEfJz&format=png&color=000000" alt="CTRL"
                    ...
                    <ul v-if="isSearchFocused && filteredNavItems.length"
                        ...
                        <li v-for="(item, index) in filteredNavItems" :key="index"
                            ...
                            <a :href="item.url" @click="searchQuery = ''; isSearchFocused = false">{{ item.name }}</a>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
                <div class="flex items-center mt-3 md:hidden">
                    <div class="relative">
                        ...
                        <input type="text" id="search-navbar-mobile" v-model="searchQuery"
                ...
                md:dark:bg-gray-900 dark:border-gray-700">
                    <li v-for="(item, index) in navItems" :key="index">
                        <a :href="item.url" @click="handleNavClick(item)"
                            class="inline-flex items-center space-x-2 py-2 px-3 rounded-sm md:p-0" :class="[
                                item.active
                            ...
                            <svg v-if="item.icon"
                                ...
                                <path v-if="item.icon === 'user'" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="2"
